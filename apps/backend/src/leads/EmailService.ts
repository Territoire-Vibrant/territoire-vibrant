...
export type EmailPort = {
  readonly sendEbook: (input: EbookEmail) => Effect.Effect<void, ExternalServiceUnavailable | ConfigurationFailure>
  readonly sendContact: (input: ContactEmail) => Effect.Effect<void, ExternalServiceUnavailable | ConfigurationFailure>
}
 ...
 * that runtime — a `import('mailersend')` anywhere in this file's import graph
...
/**
 * The isolate cannot send mail, so it fails loudly rather than silently
